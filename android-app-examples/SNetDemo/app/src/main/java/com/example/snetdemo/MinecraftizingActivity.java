...
import io.singularitynet.service.minecraftizing.MinecraftizingServiceOuterClass;

import static com.example.snetdemo.ImageUtils.*;

public class MinecraftizingActivity extends AppCompatActivity
{
    ...
    private Button btn_Minecraftize;
    ...
    private MinecraftizingService minecraftizingService;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.activity_minecraftizing);
        ...
        setTitle("Minecraftizing Demo");

        btn_UploadImageInput = (Button) findViewById(R.id.btn_uploadImageForSegmentation);
        btn_Minecraftize = (Button) findViewById(R.id.btn_minecraftize);
        btn_Minecraftize.setEnabled(false);
        ...
        AsyncTask task = new AsyncTask<Object, Object, Object>()
        {
            ...
            {
                SnetSdk sdk = new SnetSdk(MinecraftizingActivity.this);
                minecraftizingService = new MinecraftizingService(sdk, BigInteger.valueOf(channelID));
                return null;
            }
    ...
    public void sendMinecraftizeMessage(View view)
    {
        if(this.mIsInputImageUploaded)
        {
            AsyncTask task = new AsyncTask<Object, Integer, Object>()
            {
                ...
                protected Void doInBackground(Object... param)
                {
                    ...
                    try
                    {
                        bitmap = handleSamplingAndRotationBitmap(MinecraftizingActivity.this, Uri.fromFile(new File(mImageInputPath)));
                    }
                    catch (IOException e)
                    {
                    ...
                    MinecraftizingServiceOuterClass.minecraftRequest request = MinecraftizingServiceOuterClass.minecraftRequest.newBuilder()
                            .setInputImage(ByteString.copyFrom(bytesInput))
                            .setDataset("minecraft_landscapes")
                            .setNetworkName("UGATIT")
                            .build();

                    long startTime = System.nanoTime();
                    MinecraftizingServiceOuterClass.minecraftResponse response = null;

                    try
                    {
                        response = minecraftizingService.getStub().getMinecraftiziedImage(request);
                    }
                    catch (Exception e)
                    {
                    ...
                    byte[] decodedBytes = response.getOutputBytes().toByteArray();
                    byte[] decodedString = Base64.decode(response.getOutput(), Base64.DEFAULT);
                    Bitmap decodedBitmap = BitmapFactory.decodeByteArray(decodedString, 0, decodedString.length);

                    mDecodedBitmap = decodedBitmap;
                    File fr = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM), BuildConfig.APPLICATION_ID + "/minecraftized_image.jpg");
                ...
                protected void onPostExecute(Object obj)
                {
                    ...
                    if (isExceptionCaught)
                    {
                        ...
                        new AlertDialog.Builder(MinecraftizingActivity.this)
...
//                                .setNegativeButton(android.R.string.no, null)
    ...
    private void loadImageFromFileToImageView(ImageView imgView, Uri fileURI)
    {
        ...
        Glide.with(MinecraftizingActivity.this)
                .load(fileURI)
                .fitCenter()
                .into(imgView);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data)
    {
        if ( requestCode == REQUEST_CODE_UPLOAD_INPUT_IMAGE)
        {
            if(resultCode==RESULT_OK)
            {
                ...
                btn_Minecraftize.setEnabled(true);
